...
class TestRepository:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({'title': 'Naruto', 'status': AnimeStatus.completed, 'genres': ['senen'], 'description': 'just description'},
             does_not_raise()),
            ({'title': "test", 'status': AnimeStatus.ongoing, 'genres': ['test'], 'description': 'just description'},
             does_not_raise()),
            ({"wrong dict": None},
    ...
    class TestWithStartData:
        ...
        @pytest.mark.parametrize(
            ...
            [
                ...
                ({"status": "announced"}, [4]),
                ({"status": "wrong value"}, []),
                ({"genres": ["senen", 'also genres']}, [1, 2]),
                ({"genres": ["detective", 'also genres']}, [3]),
                ({"genres": ["wrong_value"]}, []),
                ({"title": "Naruto"}, [1]),
                ({"title": "Death note"}, [3]),
                ({"title": "wrong value"}, []),
                ({"status": AnimeStatus.announced, "genres": ["fantasy", 'also genres']}, [4]),
            ]
        )
